...
class FirestoreServiceImpl(private val firestore: FirebaseFirestore) : FirestoreService {
    ...
    override suspend fun <T> getDocument(collection: String, documentId: String, type: Class<T>): T? {
        ...
        } catch (e: Exception) { // Allgemeinere Exception fangen, da CancellationException spezifisch ist
    ...
    override fun <T> getCollection(clazz: Class<T>): CollectionReference {
        return when (clazz) {
            ...
            Speaker::class.java -> {
                ...
                Log.w(TAG, "Attempted to get Speaker collection as a top-level collection. Speakers are a subcollection of Congregations.")
            ...
            Speech::class.java -> firestore.collection("speeches")
                ...
                firestore.collection("unknown_collection_${clazz.simpleName}") // Zur Sicherheit, aber sollte nicht passieren
            }
        }
    }
    ...
    override fun getSpeakersSubcollection(congregationId: String): CollectionReference {
        if (congregationId.isBlank()) {
            throw IllegalArgumentException("Congregation ID cannot be blank to access speakers subcollection.")
        }
        ...
        return firestore.collection("congregations").document(congregationId).collection("speakers")
    }
    ...
    override suspend fun <T : Any> saveDocumentWithId(collectionPath: String, documentId: String, document: T): Boolean {
        return try {
    ...
    suspend fun <T : Any> saveSubCollectionDocumentWithId(
        ...
        return try {
            ...
            Log.e(TAG, "saveSubCollectionDocumentWithId to '$parentCollectionPath/$parentId/$subCollectionName/$documentId' failed: ", e)
            false
        }
    }
